...
import actions from 'store/actions/categories/categoriesListActions';
...
const CategoriesTable = () => {
  ...
  const loading = useTypedSelector((store) => store.categories.list.loading);
  const rows = useTypedSelector((store) => store.categories.list.rows);
  const count = useTypedSelector((store) => store.categories.list.count);
  const modalOpen = useTypedSelector(
    (store) => store.categories.list.modalOpen,
  );
  const idToDelete = useTypedSelector(
    (store) => store.categories.list.idToDelete,
  );
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/categories/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/categories/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget className='widget-p-md' title={<h4>Categories</h4>}>
