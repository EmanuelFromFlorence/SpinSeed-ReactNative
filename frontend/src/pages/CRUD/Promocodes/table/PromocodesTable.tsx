...
import * as dataFormat from 'pages/CRUD/Promocodes/table/PromocodesDataFormatters';

// eslint-disable-next-line
import * as productsDataFormat from 'pages/CRUD/Products/table/ProductsDataFormatters';

import actions from 'store/actions/promocodes/promocodesListActions';
...
const PromocodesTable = () => {
  ...
  const loading = useTypedSelector((store) => store.promocodes.list.loading);
  const rows = useTypedSelector((store) => store.promocodes.list.rows);
  const count = useTypedSelector((store) => store.promocodes.list.count);
  const modalOpen = useTypedSelector(
    (store) => store.promocodes.list.modalOpen,
  );
  const idToDelete = useTypedSelector(
    (store) => store.promocodes.list.idToDelete,
  );
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/promocodes/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/promocodes/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  const columns: Array<any> = [
...

    {
      ...
      formatter: productsDataFormat.listFormatter,
  ...
  return (
    <div>
      <Widget className='widget-p-md' title={<h4>Promocodes</h4>}>
        <Link to='/admin/promocodes/new'>
