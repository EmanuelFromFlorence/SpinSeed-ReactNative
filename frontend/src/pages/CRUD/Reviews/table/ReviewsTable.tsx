...
import * as dataFormat from 'pages/CRUD/Reviews/table/ReviewsDataFormatters';
...
import actions from 'store/actions/reviews/reviewsListActions';
...
const ReviewsTable = () => {
  ...
  const loading = useTypedSelector((store) => store.reviews.list.loading);
  const rows = useTypedSelector((store) => store.reviews.list.rows);
  const count = useTypedSelector((store) => store.reviews.list.count);
  const modalOpen = useTypedSelector((store) => store.reviews.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.reviews.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/reviews/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/reviews/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget className='widget-p-md' title={<h4>Reviews</h4>}>
