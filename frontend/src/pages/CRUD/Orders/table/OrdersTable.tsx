...
import actions from 'store/actions/orders/ordersListActions';
...
const OrdersTable = () => {
  ...
  const loading = useTypedSelector((store) => store.orders.list.loading);
  const rows = useTypedSelector((store) => store.orders.list.rows);
  const count = useTypedSelector((store) => store.orders.list.count);
  const modalOpen = useTypedSelector((store) => store.orders.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.orders.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color='default'
            size='xs'
            onClick={() => dispatch(push(`/admin/orders/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color='info'
          size='xs'
          onClick={() => history.push(`/admin/orders/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget className='widget-p-md' title={<h4>Orders</h4>}>
